...
public class MegaFlowHandles
{
	public static Vector3 FreeMoveHandle(Vector3 pos, Quaternion rot, float size, Vector3 snap)
	{
#if UNITY_5_6 || UNITY_2017 || UNITY_2018 || UNITY_2019 || UNITY_2020
		return Handles.FreeMoveHandle(pos, rot, size, snap, Handles.DotHandleCap);
#else
		return Handles.FreeMoveHandle(pos, rot, size, snap, Handles.DotCap);
#endif
	}

	public static Vector3 FreeMoveHandleSphere(Vector3 pos, Quaternion rot, float size, Vector3 snap)
	{
#if UNITY_5_6 || UNITY_2017 || UNITY_2018 || UNITY_2019 || UNITY_2020
		return Handles.FreeMoveHandle(pos, rot, size, snap, Handles.SphereHandleCap);
#else
		return Handles.FreeMoveHandle(pos, rot, size, snap, Handles.SphereCap);
#endif
	}
...
public class MegaFlowEditor : Editor
{
	...
	public void LoadNetCDFFrame(MegaFlow flow, string filename)
	{
		if ( flow.frames.Count > 0 )
		{
			if ( !EditorUtility.DisplayDialog("Add to or Replace", "Add new frame to existing frames", "Add", "Replace") )
		...
		MegaFlowFrame frame = MegaFlowFGA.LoadFrameNetCDF(filename);
		if ( frame != null )
			flow.AddFrame(frame);
	}
	...
	public override void OnInspectorGUI()
	{
		...
		switch ( mod.datasource )
		{
			...
			case MegaFlowDataSource.FGA:
				...
				else
				{
					...
					if ( GUILayout.Button("Load NetCDF File") )
					{
						string file = EditorUtility.OpenFilePanel("NetCDF Vector Field File", lastpath, "nc");

						if ( file != null && file.Length > 1 )
						{
							lastpath = file;
							LoadNetCDFFrame(mod, file);
							mod.SetFrame(0);
							mod.CalcMemUse();
						}
					}
		...
		if ( mod.showadvparams )
		{
			...
			if ( mod.flow )
			{
				...
				//if ( GUILayout.Button("Export NetCDF") )
				//{
					//string epath = EditorUtility.SaveFilePanel("Export NetCDF File", "", "NetCDFexport_" + mod.frame, "nc");

					//if ( epath.Length > 0 )
						//MegaFlowFGA.SaveNetCDF(mod.flow, epath);
				//}
			}
	...
	public void OnSceneGUI()
	{
		...
		if ( mod.showRibbon )
		{
			...
			Vector3 hp1 = MegaFlowHandles.FreeMoveHandle(hp, Quaternion.identity, (mod.SizeY + mod.SizeZ) * 0.5f * 0.025f, Vector3.zero);	//, Handles.DotCap);
			...
			hp1 = MegaFlowHandles.FreeMoveHandle(hp, Quaternion.identity, (mod.SizeY + mod.SizeZ) * 0.5f * 0.025f, Vector3.zero);	//, Handles.DotCap);
		...
		Vector3 scale = mod.transform.lossyScale;

		Handles.color = Color.green;
		switch ( mod.Plane )
		{
			case MegaFlowAxis.X:
				mod.handlepos = MegaFlowHandles.FreeMoveHandleSphere(mod.handlepos, Quaternion.identity, mod.flow.spacing.x * scale.x, Vector3.zero);	//, Handles.SphereCap);
			...
			case MegaFlowAxis.Y:
				mod.handlepos = MegaFlowHandles.FreeMoveHandleSphere(mod.handlepos, Quaternion.identity, mod.flow.spacing.y * scale.y, Vector3.zero);	//, Handles.SphereCap);
			...
			case MegaFlowAxis.Z:
				mod.handlepos = MegaFlowHandles.FreeMoveHandleSphere(mod.handlepos, Quaternion.identity, mod.flow.spacing.z * scale.z, Vector3.zero);	//, Handles.SphereCap);
	...
	public static void CreateCellMaterial()
	{
		if( !cellMaterial )
		{
			...
			//"} } }" );
			cellMaterial = new Material(Shader.Find("Lines/Colored Blended"));
